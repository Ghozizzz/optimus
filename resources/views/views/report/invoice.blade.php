      ...
      <tr>
        <td colspan="5">
          @if($session['proforma_invoice']->incoterm == 'FOB')
            <!-- do nothing -->
          @else 
            Ocean freight : {{$session['proforma_invoice']->currency}} {{number_format($session['proforma_invoice']->shipping_price,2,'.',',')}}
          @endif
        </td>
      </tr>
...
<!--      <tr>
      ...
      <tr>
        <td colspan="5">
          @if($session['proforma_invoice']->incoterm == 'FOB')
            Car Price : USD {{number_format($session['proforma_invoice']->total_amount,2,'.',',')}}
          @elseif($session['proforma_invoice']->incoterm == 'FOB')
            
            <?php 
            $car_price = $session['proforma_invoice']->total_amount - $session['negotiation']->insurance_fee - $session['proforma_invoice']->shipping_price;
            if($session['proforma_invoice']->inspection == 1){
              $car_price -= $session['negotiation']->inspection_fee;
            }?>
            
            Car Price : USD {{number_format($car_price,2,'.',',')}}
          @else
          <!--cif-->
