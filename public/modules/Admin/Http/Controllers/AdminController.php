...
class AdminController extends Controller {
	...
	public function index()
	{
		...
		return view('admin::index')->with($optimus);
	}
	/*Smtp*/
...
//	        dd($e);
	...
	public function dealerIndex(){
		...
		$db = adminModel::dealer($getMax, $descasc, $page);
		if(in_array(Session::get('login_type'),['admin','super admin'])) {
			return view('admin::dealer', compact('db','getMax','descasc'))->with($optimus);
		}else{
  ...
  public function bannerIndex(){
		...
		if(isset($_GET['descasc'])) $descasc = $_GET['descasc'];
		if(isset($_GET['page'])) $page = ($_GET['page'] - 1) * $getMax;
		$db = adminModel::banner($getMax, $descasc, $page);
		if(in_array(Session::get('login_type'),['admin','super admin'])) {
			return view('admin::banner', compact('db','getMax','descasc'))->with($optimus);;
		}else{
			echo '<script>alert("You dont have permission here");</script>';
			return redirect('admin');
		}
  ...
  /*port destination*/
	public function portDestinationIndex(){
		...
		if(isset($_GET['descasc'])) $descasc = $_GET['descasc'];
		if(isset($_GET['page'])) $page = ($_GET['page'] - 1) * $getMax;
		$optimus['db'] = adminModel::portDestination($getMax, $descasc, $page);
		$optimus['country'] = adminModel::country();
    if(in_array(Session::get('login_type'),['admin','super admin','sales'])) {
			return view('admin::port_destination', compact('getMax','descasc'))->with($optimus);;
		}else{
			echo '<script>alert("You dont have permission here");</script>';
			return redirect('admin');
		}
	...
	public function portDestinationUpdate($id, Request $request){
		...
		if ($validator->passes()) {
			...
			if (!empty($request->country_name))$data['country_name'] = $request->country_name;
  ...
  public function portDischargeView($id){
	...
	public function customerIndex(){
		...
		if(isset($_GET['descasc'])) $descasc = $_GET['descasc'];
		if(isset($_GET['page'])) $page = ($_GET['page'] - 1) * $getMax;
		$db = adminModel::customer($getMax, $descasc, $page);
		if(in_array(Session::get('login_type'),['admin','super admin','sales'])) {
			return view('admin::customer', compact('db','getMax','descasc'))->with($optimus);;
		}else{
			echo '<script>alert("You dont have permission here");</script>';
			return redirect('admin');
		}
	...
	public function salesIndex(){
		...
		if(isset($_GET['descasc'])) $descasc = $_GET['descasc'];
		if(isset($_GET['page'])) $page = ($_GET['page'] - 1) * $getMax;
		$db = adminModel::sales($getMax, $descasc, $page);
		if(in_array(Session::get('login_type'),['admin','super admin'])) {
			return view('admin::sales', compact('db','getMax','descasc'))->with($optimus);;
		}else{
			echo '<script>alert("You dont have permission here");</script>';
			return redirect('admin');
		}
	...
	public function logisticIndex(){
		...
		if(isset($_GET['descasc'])) $descasc = $_GET['descasc'];
		if(isset($_GET['page'])) $page = ($_GET['page'] - 1) * $getMax;
		$db = adminModel::logistic($getMax, $descasc, $page);
		if(in_array(Session::get('login_type'),['admin','super admin'])) {
			return view('admin::logistic', compact('db','getMax','descasc'))->with($optimus);;
		}else{
			echo '<script>alert("You dont have permission here");</script>';
			return redirect('admin');
		}
	...
	public function userIndex(){
		...
		if(isset($_GET['descasc'])) $descasc = $_GET['descasc'];
		if(isset($_GET['page'])) $page = ($_GET['page'] - 1) * $getMax;
		$db = adminModel::user($getMax, $descasc, $page);
		if(in_array(Session::get('login_type'),['admin','super admin'])) {
			return view('admin::user', compact('db','getMax','descasc'))->with($optimus);
		}else{
			echo '<script>alert("You dont have permission here");</script>';
			return redirect('admin');
		}
	...
	public function carIndex(){
		...
		if(isset($_GET['descasc'])) $descasc = $_GET['descasc'];
		if(isset($_GET['page'])) $page = ($_GET['page'] - 1) * $getMax;
		$db = adminModel::car($getMax, $descasc, $page);
		if(in_array(Session::get('login_type'),['admin','super admin'])) {
			return view('admin::car', compact('db','getMax','descasc'))->with($optimus);
		}else{
			echo '<script>alert("You dont have permission here");</script>';
			return redirect('admin');
		}
  ...
  public function postUpload(Request $request){
    ...
    $watermark = 'OPTIMUS';
    Image::make($file->getRealPath())->fit(250, 250)->text($watermark, 25, 25, function($font) {
                    $font->size(45);
  ...
  public function deleteUpload(Request $request)
  {
	...
	public function carSave(Request $request){
		...
		if ($validator->passes()){			
			$data = [
				...
				'registration_year' => $request->newRegistrationYear,
				'registration_month' => $request->newRegistrationMonth,
	...
	$validator = Validator::make([
		...
		if ($validator->passes()) {
			...
			if (!empty($request->editRegistrationYear))$data['registration_year'] = $request->editRegistrationYear;
			...
			if (!empty($request->editRegistrationMonth))$data['registration_month'] = $request->editRegistrationMonth;
	...
	public function carModelIndex(){
		...
		if(isset($_GET['descasc'])) $descasc = $_GET['descasc'];
		if(isset($_GET['page'])) $page = ($_GET['page'] - 1) * $getMax;
		$db = adminModel::carModel($getMax, $descasc, $page);
		if(in_array(Session::get('login_type'),['admin','super admin'])) {
			return view('admin::carmodel', compact('db','getMax','descasc', 'page'))->with($optimus);
		}else{
			echo '<script>alert("You dont have permission here");</script>';
			return redirect('admin');
		}
	...
	public function carMakeIndex(){
		...
		if(isset($_GET['descasc'])) $descasc = $_GET['descasc'];
		if(isset($_GET['page'])) $page = ($_GET['page'] - 1) * $getMax;
		$db = adminModel::carMake($getMax, $descasc, $page);
		if(in_array(Session::get('login_type'),['admin','super admin'])) {
			return view('admin::carmake', compact('db','getMax','descasc'))->with($optimus);
		}else{
			echo '<script>alert("You dont have permission here");</script>';
			return redirect('admin');
		}
	...
	public function carMakeSave(Request $request){
		...
		if ($validator->passes()) {		    
			...
			if ($request->hasFile('newLogo')) {
				...
				$name = time().'.'.$image->getClientOriginalExtension();
				$destinationPath = public_path('/images/carmake');
				$image->move($destinationPath, $name);
				$img = '/images/carmake/'.time().'.'.$image->getClientOriginalExtension();
			}
			$data = [
				'make' => $request->newMake,
				'corporate' => $request->newCorporate,
				'logo' => $img
			];
	...
	public function carMakeUpdate($id, Request $request){
		...
		if ($validator->passes()) {
			...
			if (!empty($request->editCorporate))$data['corporate'] = $request->editCorporate;
	...
	public function carEngineIndex(){
		...
		if(isset($_GET['descasc'])) $descasc = $_GET['descasc'];
		if(isset($_GET['page'])) $page = ($_GET['page'] - 1) * $getMax;
		$db = adminModel::carEngine($getMax, $descasc, $page);
		if(in_array(Session::get('login_type'),['admin','super admin'])) {
			return view('admin::carengine', compact('db','getMax','descasc'))->with($optimus);
		}else{
			echo '<script>alert("You dont have permission here");</script>';
			return redirect('admin');
		}
	...
	public function negotiationIndex(){
		...
		if(isset($_GET['descasc'])) $descasc = $_GET['descasc'];
		if(isset($_GET['page'])) $page = ($_GET['page'] - 1) * $getMax;
		$db = adminModel::negotiation($getMax, $descasc, $page);
    if(in_array(Session::get('login_type'),['admin','super admin','sales','super sales'])) {
      return view('admin::negotiation', compact('db','getMax','descasc'))->with($optimus);
		}else{
			echo '<script>alert("You dont have permission here");</script>';
			return redirect('admin');
		}
    ...
    $active_menu = 'dashboard';
		...
		return view('admin::negotiationdetail', compact('session','active_menu','negotiation','negotiation_line'))->with($optimus);
	}
  ...
  public function documentCopies(){
    ...
    $optimus['config'] = API::getDefaultConfig();
    return view('admin::document_copies', compact('session','document'))->with($optimus);
  }
  ...
  public function documentUpload(Request $request){
    ...
    $getMax = isset($_GET['max']) ? $_GET['max'] : 5;
		if(isset($_GET['descasc'])) $descasc = $_GET['descasc'];
		if(isset($_GET['page'])) $page = ($_GET['page'] - 1) * $getMax;
		$optimus['db'] = adminModel::paymentInvoiceGroup($getMax, $descasc, $page);
    
    $invoice_list = [];
    if(count($optimus['db']['data'])>0){
      foreach($optimus['db']['data'] as $detail){
        array_push($invoice_list, $detail->id);
        $optimus['invoice'][$detail->id]['data'] = $detail;
      }
    }
    $optimus['invoice_detail'] = adminModel::paymentInvoiceDetail($invoice_list);
    
    if(count($optimus['invoice_detail'])>0){
      foreach($optimus['invoice_detail'] as $detail){
        $optimus['invoice'][$detail->id]['invoice'][] = $detail;
      }
    }

		if(in_array(Session::get('login_type'),['admin','super admin', 'finance'])) {
			return view('admin::invoice', compact('getMax','descasc'))->with($optimus);
		}else{
			echo '<script>alert("You dont have permission here");</script>';
			return redirect('admin');
		}
  ...
  public function proformainvoice($id){
    ...
    return view('admin::proforma_invoice', compact('session','invoice','car','countries','logistic'))->with($optimus);     
  }

	public function cancelProformaInvoice($invoice_id){
    $session = Session::get(null);
    $proforma_invoice = $session['proforma_invoice'];
    if(count($proforma_invoice)>0){
      $arr = [
          ...
          'departure_port' => $proforma_invoice->port_departure, 
          'destination_port' => $proforma_invoice->port_destination, 
          'country' => $proforma_invoice->country_code, 
  ...
  public function saveProformaInvoice(Request $request){
    ...
    $arr = [
        ...
        'departure_port' => isset($param['departure_port'])?$param['departure_port']:'', 
        'destination_port' => isset($param['destination_port'])?$param['destination_port']:'', 
  ...
  public function invoice($id){
    ...
    return view('admin::negotiation_invoice', compact('session','invoice','countries','logistic','car'))->with($optimus);     
  }
  ...
  function paymentConfirmation($invoice_id){
    ...
    return view('admin::payment_confirmation', compact('session','invoice','payment'))->with($optimus);
  }
  ...
  function trackInvoice(){
    $session = Session::get(null);  
    $optimus['config'] = API::getDefaultConfig();
    return view('admin::tracking_invoice', compact('session'))->with($optimus);
  }
  ...
  function settingIndex(){
		...
		if(isset($_GET['descasc'])) $descasc = $_GET['descasc'];
		if(isset($_GET['page'])) $page = ($_GET['page'] - 1) * $getMax;
		$db = adminModel::settingIndex($getMax, $descasc, $page);
		if(in_array(Session::get('login_type'),['admin','super admin'])) {
			return view('admin::setting', compact('db','getMax','descasc'))->with($optimus);
		}else{
			echo '<script>alert("You dont have permission here");</script>';
			return redirect('admin');
		}
  ...
  function receiveItem(){
    ...
    $optimus['review'] = Front_model::getReview($session['invoice']->id);
    $optimus['config'] = API::getDefaultConfig();
    
    if(count($optimus['review'])== 0){
      $review = [];
    }

    return view('admin::receive_item', compact('session'))->with($optimus);
  }
}
