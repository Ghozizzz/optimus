...
function saveData(){
	...
	data.append('port_name', $('#port_name').val());
	$.ajax({  
	    url: "{{ URL::to('/') }}/admin/port-destination/save",  
	    type: "POST",  
	    data:  data,
	    processData: false,  
	    contentType: false, 
...
$('#editModal').on('show.bs.modal', function (event) {
  ...
  var modal = $(this)
	 $.get( "{{ URL::to('/') }}/admin/port-destination/view/"+ids, function( response ) {
...
function updateData(){
	...
	$.ajax({  
	    url: "{{ URL::to('/') }}/admin/port-destination/update/"+id,  
	    type: "POST",  
	    data:  data,
	    processData: false,  
	    contentType: false, 
...
function deleteData(id){
	...
	.then((willDelete) => {
 	...
 	$.get( "{{ URL::to('/') }}/admin/port-destination/delete/"+id, function( response ) {
		location.reload();
	});
