...
<div class="full-container">
            ...
            <div class="row">
              <div class="col-md-12 col-xs-12">
                <label class="col-form-label">Make:</label>
                  <select class="form-control" id="newMakeId" width="100%">
                    @foreach($make['data'] as $make_detail)
                      <option value='{{$make_detail->id}}'>{{$make_detail->make}}</option>
                    @endforeach
                  </select> 
              </div>
            </div>
        ...
        <div class="modal-body">
          <form>
            <div class="form-group">
              <label class="col-form-label">Make ID:</label>
              <select class="form-control" id="editMakeId">
                @foreach($make['data'] as $make_detail)
                  <option value='{{$make_detail->id}}'>{{$make_detail->make}}</option>
                @endforeach
              </select>
  ...
  <script>
    ...
    function saveData(){
      ...
      data.append('newMakeId', $('#newMakeId').val());
      data.append('newModel', $('#newModel').val());
      $.ajax({
      url: "{{ URL::to('/') }}/admin/carmodel/save",
              type: "POST",
              data:  data,
              processData: false,
              contentType: false,
    ...
    var modal = $(this)
            $.get("{{ URL::to('/') }}/admin/carmodel/view/" + ids, function(response) {
            console.log(response);
            modal.find('#editMakeId').val(response.make_id);
            modal.find('#editModel').val(response.model);
            });
    })
    ...
    function updateData(){
            var id = $('#idEdit').val();
            var data = new FormData();
            data.append('editMakeId', $('#editMakeId').val());
            data.append('editModel', $('#editModel').val());
            $.ajax({
            url: "{{ URL::to('/') }}/admin/carmodel/update/" + id,
                    type: "POST",
                    data:  data,
                    processData: false,
                    contentType: false,
    ...
    title: "Are you sure?",
            ...
            $.get("{{ URL::to('/') }}/admin/carmodel/delete/" + id, function(response) {
            location.reload();
            });
            } else {
