  ...
  <div class='col-md-12'>
    <caption>
      <?php if($thispage>1){
        ...
        ?>
      <a href="{{ $web_url . '?' . http_build_query($qs) }}" id="prev"><button class="btn btn-danger bottom10">Prev</div></a>
    <?php }?>
<?php if($thispage < $total_page){
      $qs['page'] = $thispage+1;
        ...
        $qs['descasc'] = isset($_GET['descasc']) ? $_GET['descasc'] : 'asc';
  ?>
      <a href="{{ $web_url . '?' . http_build_query($qs) }}" id="next"><button class="btn btn-danger bottom10">Next</div></a>
    </caption>
<?php }?>     
  </div>
</div>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form>
          <div class="form-group">
            <center><img width="100" height="100" id="editImage" src=""/></center>
...
function saveData(){
  ...
  $.ajax({  
      url: "{{ URL::to('/') }}/admin/logistic/save",  
      type: "POST",  
      data:  data,
      processData: false,  
      contentType: false, 
...
$('#editModal').on('show.bs.modal', function (event) {
  ...
  var modal = $(this)
   $.get( "{{ URL::to('/') }}/admin/logistic/view/"+ids, function( response ) {
    ...
    if(response.photo !== null){
      ...
      modal.find('#editImage').attr('src', "{{ URL::to('/') }}/uploads/logistic/" + response.photo);
    }    
  });
})
...
function updateData(){
  ...
  $.ajax({  
      url: "{{ URL::to('/') }}/admin/logistic/update/"+id,  
      type: "POST",  
      data:  data,
      processData: false,  
      contentType: false, 
...
function deleteData(id){
  ...
  $.get( "{{ URL::to('/') }}/admin/logistic/delete/"+id, function( response ) {
    location.reload();
  });
